...
inline void register_peer_dynamic(const uint8_t *mac) {
  if (!esphome::espnow::global_esp_now) {
    ESP_LOGW("espnow_disc", "global_esp_now not ready, skipping register_peer_dynamic");
    return;
  }
  if (is_local_mac(mac)) return; // Don't register ourselves
  ...
  if (current_list.find(mac_str) == std::string::npos) {
    ...
    ESP_LOGI("espnow_disc", "New peer discovered and saved: %s", mac_str.c_str());
  }
  ...
  if (err == ESP_OK) {
    ESP_LOGI("espnow_disc", "Peer %s registered (channel: current)", mac_str.c_str());
  } else {
    ESP_LOGW("espnow_disc", "Failed to register peer %s: %d", mac_str.c_str(), err);
  }
}

/** @brief Loads all peers saved in the runtime string cache. */
inline void load_peers_from_runtime_cache() {
  if (!esphome::espnow::global_esp_now || espnow_peers == nullptr) return;
  std::string current_list = espnow_peers->value();
...
inline void send_discovery_broadcast() {
  ...
  char buffer[64];
  int written = snprintf(buffer, sizeof(buffer), "ROOM_DISC:%d:%d", (int)config_floor_id->state, (int)config_room_id->state);
...
inline void send_discovery_confirmation(const uint8_t *target_mac) {
  ...
  char buffer[64];
  int written = snprintf(buffer, sizeof(buffer), "ROOM_CONF:%d:%d", (int)config_floor_id->state, (int)config_room_id->state);
  if (written < 0 || written >= sizeof(buffer)) {
    ESP_LOGE("espnow_disc", "Buffer overflow prevented in confirmation message");
    return;
  }
...
inline void send_sync_to_all_peers(const std::vector<uint8_t> &data) {
  ...
  while (true) {
    ...
    auto mac = parse_mac_local(mac_str);
    if (mac.has_value()) {
      esphome::espnow::global_esp_now->send(mac->data(), data, [](esp_err_t err) {});
    }
    if (end == std::string::npos) break;
    start = end + 1;
    end = current_list.find(",", start);
...
inline bool handle_discovery_payload(const std::string &payload, const uint8_t *src_addr) {
  ...
  if (payload.find("ROOM_DISC:") == 0 || payload.find("ROOM_CONF:") == 0) {
    if (is_local_mac(src_addr)) return true;
    
    int floor, room;
    if (sscanf(payload.c_str() + 10, "%d:%d", &floor, &room) == 2) {
      if (floor == (int)config_floor_id->state && room == (int)config_room_id->state) {
        ESP_LOGI("espnow_disc", "Matching room discovery from %02X:%02X:%02X", src_addr[3], src_addr[4], src_addr[5]);
...
namespace espnow_handler {
  ...
  inline bool validate_packet(const std::vector<uint8_t> &data) {
    ...
    if (pkt->magic_header != 0x42 || pkt->protocol_version != esphome::PROTOCOL_VERSION) {
      ESP_LOGI("vent_sync", "Invalid packet header: %d %d", pkt->magic_header, pkt->protocol_version);
      return false;
    }
    return true;
  }
  ...
  inline void handle_status_request(const esphome::VentilationPacket *pkt) {
    if (config_floor_id != nullptr && config_room_id != nullptr &&
        pkt->floor_id == (int)config_floor_id->state && pkt->room_id == (int)config_room_id->state) {
      ESP_LOGI("vent_sync", "Status request from peer %d. Sending response...", pkt->device_id);
      auto resp = build_and_populate_packet(esphome::MSG_STATUS_RESPONSE);
      if (esphome::espnow::global_esp_now) {
        esphome::espnow::global_esp_now->send(BROADCAST_MAC, resp, [](esp_err_t err) {});
      }
    }
  }
  ...
  inline void handle_config_sync(const esphome::VentilationPacket *pkt) {
    ...
    if (co2_auto_enabled != nullptr && automatik_co2 != nullptr && pkt->co2_auto_enabled != co2_auto_enabled->value()) {
    ...
    if (automatik_min_fan_level != nullptr && automatik_min_luefterstufe != nullptr &&
    ...
    if (automatik_max_fan_level != nullptr && automatik_max_luefterstufe != nullptr &&
    ...
    if (auto_co2_threshold_val != nullptr && auto_co2_threshold != nullptr &&
        pkt->auto_co2_threshold_val >= 400 && pkt->auto_co2_threshold_val <= 5000 &&
    ...
    if (auto_humidity_threshold_val != nullptr && auto_humidity_threshold != nullptr &&
        pkt->auto_humidity_threshold_val >= 0 && pkt->auto_humidity_threshold_val <= 100 &&
        pkt->auto_humidity_threshold_val != auto_humidity_threshold_val->value()) {
    ...
    // 5. System Timers
    if (sync_interval_config != nullptr &&
        pkt->sync_interval_min >= 1 && pkt->sync_interval_min <= 1440 &&
    ...
    if (vent_timer != nullptr &&
        pkt->vent_timer_min <= 1440 &&
        (uint32_t)(pkt->vent_timer_min * 60 * 1000) != v->state_machine.ventilation_duration_ms) {
    ...
    if (max_led_brightness != nullptr && led_max_brightness_config != nullptr && update_leds != nullptr &&
        pkt->max_led_brightness >= 0.05f && pkt->max_led_brightness <= 1.0f &&
  ...
  inline void handle_state_sync(const esphome::VentilationPacket *pkt) {
    ...
    if (ui_active != nullptr && ui_timeout_script != nullptr && update_leds != nullptr && fan_speed_update != nullptr) {
...
/**
 * @brief Thread safety note: ESPHome's internal architecture buffers ESP-NOW events 
 * and executes lambda callbacks within the main loop() context. Global variable modifications 
 * here are safe from concurrent HW interrupts.
 */
inline void handle_espnow_receive(const std::vector<uint8_t> &data) {
  if (!espnow_handler::validate_packet(data)) return;

  const auto *pkt = reinterpret_cast<const esphome::VentilationPacket *>(data.data());
  auto *v = ventilation_ctrl;
  if (v == nullptr) return;

  if (static_cast<esphome::MessageType>(pkt->msg_type) == esphome::MSG_STATUS_REQUEST) {
  ...
  if (static_cast<esphome::MessageType>(pkt->msg_type) == esphome::MSG_STATUS_RESPONSE && !v->is_state_synced) {
    if (config_floor_id != nullptr && config_room_id != nullptr &&
        pkt->floor_id == (int)config_floor_id->state && pkt->room_id == (int)config_room_id->state) {
      ESP_LOGI("vent_sync", "Adopting state from peer %d (REBOOT SYNC)", pkt->device_id);

      int mode_idx = 1;
      if (pkt->current_mode == esphome::MODE_VENTILATION) mode_idx = 2;
      else if (pkt->current_mode == esphome::MODE_STOSSLUEFTUNG) mode_idx = 3;
      else if (pkt->current_mode == esphome::MODE_OFF) mode_idx = 4;

      cycle_operating_mode(mode_idx);
      
      if (fan_intensity_level != nullptr) fan_intensity_level->value() = 0;
      if (fan_speed_update != nullptr) fan_speed_update->execute();
      
      v->set_fan_intensity(pkt->fan_intensity);
      if (fan_intensity_level != nullptr) fan_intensity_level->value() = pkt->fan_intensity;
