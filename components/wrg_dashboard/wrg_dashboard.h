...
class WrgDashboard : public Component, public AsyncWebHandler {
 public:
  ...
  void set_heat_recovery_efficiency(sensor::Sensor *s)       { heat_recovery_efficiency_ = s; }
  void set_fan_rpm(sensor::Sensor *s)                        { fan_rpm_ = s; }
  void set_filter_operating_days(sensor::Sensor *s)          { filter_operating_days_ = s; }

  void set_filter_change_alarm(binary_sensor::BinarySensor *b) { filter_change_alarm_ = b; }
  void set_radar_presence(binary_sensor::BinarySensor *b)      { radar_presence_ = b; }

  void set_scd41_co2_bewertung(text_sensor::TextSensor *t)     { scd41_co2_bewertung_ = t; }

  void set_test_speed_slider(number::Number *n)                { test_speed_slider_ = n; }
  void set_vent_timer(number::Number *n)                       { vent_timer_ = n; }
  void set_sync_interval_config(number::Number *n)             { sync_interval_config_ = n; }
  void set_fan_intensity_display(number::Number *n)            { fan_intensity_display_ = n; }
  void set_automatik_min_luefterstufe(number::Number *n)       { automatik_min_luefterstufe_ = n; }
  void set_automatik_max_luefterstufe(number::Number *n)       { automatik_max_luefterstufe_ = n; }
  void set_auto_co2_threshold(number::Number *n)               { auto_co2_threshold_ = n; }
  void set_auto_humidity_threshold(number::Number *n)          { auto_humidity_threshold_ = n; }
  void set_auto_presence_slider(number::Number *n)             { auto_presence_slider_ = n; }
