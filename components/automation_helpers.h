  ...
  if (!std::isnan(eff_in) && !std::isnan(eff_out)) {
    if (internal_mode != esphome::MODE_VENTILATION) {
      ...
      if (eff_out >= eff_in) {
        ...
        ESP_LOGI(
            "auto_mode",
            "Summer cooling aborted (Too warm outside): In=%.1fC, Out=%.1fC",
            eff_in, eff_out);
      }
    }
  } else if (internal_mode != esphome::MODE_ECO_RECOVERY &&
...
inline float get_current_target_speed() {
  ...
  if (auto_mode_active != nullptr && auto_mode_active->value()) {
    ...
    if (ventilation_ctrl != nullptr) {
      const uint32_t now = millis();
      if (!std::isnan(ventilation_ctrl->last_peer_pid_demand) &&
...
inline void update_filter_analytics() {
  ...
  if (id(filter_last_change_ts) == 0 && now.is_valid()) {
    ...
    ESP_LOGI("filter", "Initial filter timestamp set: %lld",
...
inline void handle_espnow_receive(const std::vector<uint8_t> &data) {
  ...
  // If any configs changed, re-evaluate standard logic immediately to reflect
  // new boundaries.
  if (dirty) {
    evaluate_auto_mode();
  }
