...
10. **Room-wide fusion & Smart Climate Control inputs**: T-7k–T-7p — room CO2 / demand / humidity fusion (freshness, no feedback loop), AC state sources (HA push expiry, API link, peer flag), configuration ranges and fusion window, Smart-Automatik level mapping that always enforces the level window (HVAC cap regression).
