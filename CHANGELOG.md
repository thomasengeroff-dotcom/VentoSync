...
> **Breaking (Smart Climate Control):** ESP-NOW protocol **v10** — flash **all devices of a room** together. The AC state now comes from a Home Assistant **automation** calling the API action `set_ac_active` (the imported `binary_sensor.ventosync_hvac_active_room_<room_id>` is no longer read). The "Klima-Koordination" switch starts **off** after the update — switch it on once per room. Setup: `documentation/en/en_smart-climate-control.md`.
...
- ESP-NOW `VentilationPacket::hvac_flags` (protocol v10): bit 0 room-wide switch, bit 1 the sender's own HA AC state (never a fused value).
- Unit tests T-7n (AC state sources: expiry, API link, peers), T-7o (config ranges, fusion window), T-7p (level window regression).

### Removed

- Substitution `hvac_ac_sensor_id` and the imported HA binary sensor `hvac_ac_active` (replaced by the API action).
...
- **ESPHome 2026.8.0 Migration**:
  - Migrated deprecated `platformio_options.build_flags` to `esphome.build_flags` across all 4 hardware variant configurations (`ventosync_nosensor.yaml`, `ventosync_NTConly.yaml`, `ventosync_bme680_only.yaml`, `ventosync_radar_only.yaml`).
  - Removed legacy PlatformIO configuration blocks (including `extra_scripts`) from `packages/base/esp32c6_common.yaml` to align with native ESP-IDF toolchain.
  - Simplified `text_project_version` sensor to directly consume `${project_version}`.
  - Automated build version bumping for ESP-IDF builds via `upload_all.sh`, CLI wrapper `esphome-build`, and automated Git `pre-commit` hook (`.git/hooks/pre-commit`).
- **Documentation Modularization**: Streamlined `Readme.md` and `Readme_de.md` by modularizing and extracting comprehensive guides into dedicated Markdown files under `documentation/` (Control Panel Operation, Comfort & Safety Features, ESP-NOW Mesh Communication, Local Web Dashboard, Window Guard HA Setup, Roadmap).

### Fixed

- **Compiler Warnings & Build Cleanliness** (ESP-IDF 5.5.5 / GCC 14):
  - Added `-Wno-address` to `esphome.build_flags` in `esp32c6_common.yaml` to suppress `-Waddress` warnings generated by ESPHome's placement-new component storage null-guards.
  - Resolved 32-bit RISC-V `-Wformat` format specifier warnings (`%u`/`%d` expecting `unsigned int` instead of `uint32_t`/`unsigned long`) across `climate.h`, `automation_helpers.h`, `ventilation_group.h`, and `sensor_hrv_efficiency.yaml`.
  - Fixed `-Wtype-limits` unsigned comparison warning (`>= 0`) on `pkt->auto_humidity_threshold_val` in `network_sync.h`.
